...
public class AdvertisingUI extends UI implements ResponseCallBack<SuccessResponse, ExceptionResponse> {
    ...
    private Collection<SideBarMenu.MenuItem> getSideBarMenuItems() {
        ...
        items.add(new SideBarMenu.MenuItem("dashboard", "DashBoard", FontAwesome.HOME));
    ...
    private void updateContent() {
        ...
        if (user != null) {
            ...
            mainView.initializeUserMenu(user.username(), getMenuBarItems());
            setContent(mainView);
        } else {
        ...
        if (response instanceof AbstractAuthCommandResponse) {
            if (response instanceof AuthSuccessComandResponse) {
                ...
                if (session != null) {
                    ...
                    if (isRememberMe) {
                        Cookie cookie = new Cookie(Attributes.REMEMBER_ME.value(), user == null ? "" : user.userToken());
